...
static const uint8_t magicBT[4] =
{
    64,27,103,119
};
        ...
        uint8_t cmd, const void *pData, uint8_t lengthOfData)
{
    ...
    memset(pProtocal->package, 0, sizeof(pProtocal->package));

//    LOGD("make package for device:%d", pProtocal->sendDevice);
    pMagic = NULL;
    if (pProtocal->sendDevice == DEVICE_TYPE_BT)
    {
        pMagic = magicBT;
    }
    else if (pProtocal->sendDevice == DEVICE_TYPE_EV)
    {
        pMagic = magicDevice;
    }
    ...
    pProtocal->package[0] = pMagic[0];
    pProtocal->package[1] = pMagic[1];
    pProtocal->package[2] = pMagic[2];
    pProtocal->package[3] = pMagic[3];
...
int32_t protocalCheckHeader(ProtocalStatusType *pProtocal, uint8_t ch)
{
    ...
    if (memcmp(pHeader, magicBT, 4) == 0)
    {
        deviceMatch = DEVICE_TYPE_BT;
    }

    if (memcmp(pHeader, magicDevice, 4) == 0)
    {
        deviceMatch = DEVICE_TYPE_EV;
    }
